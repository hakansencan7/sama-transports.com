...
LOGIN_ATTEMPTS_DIR = Path.home() / "sama_security"
LOGIN_MAX_ATTEMPTS = 5
LOGIN_LOCK_SECONDS = 600

def login_attempt_key(ip, username):
    raw = f"{ip}|{username}".encode("utf-8")
    return hashlib.sha256(raw).hexdigest()


def login_attempt_file(ip, username):
    return LOGIN_ATTEMPTS_DIR / f"{login_attempt_key(ip, username)}.json"


def get_login_attempts(ip, username):
    path = login_attempt_file(ip, username)

    if not path.exists():
        return {"failures": 0, "first_failure": 0, "lock_until": 0}

    try:
        data = json.loads(path.read_text())
        return {
            "failures": int(data.get("failures", 0)),
            "first_failure": int(data.get("first_failure", 0)),
...
def save_login_attempts(ip, username, data):
    LOGIN_ATTEMPTS_DIR.mkdir(mode=0o700, exist_ok=True)
...
def clear_login_attempts(ip, username):
    try:
        login_attempt_file(ip, username).unlink()
...
async def login_submit(request: Request):
    try:
        ...
        attempt_data = get_login_attempts(client_ip, username)
        now = int(time.time())

        if attempt_data["lock_until"] > now:
            remaining = attempt_data["lock_until"] - now
            return Response(
                content=f"Too many failed attempts. Try again in {remaining // 60 + 1} minutes.",
                status_code=429,
                headers={"Retry-After": str(remaining)}
            )

        # LOGIN_LOCK_CHECK_ADDED
        ...
        if (
            ...
            if attempt_data["first_failure"] == 0:
                attempt_data["first_failure"] = now

            if attempt_data["failures"] >= LOGIN_MAX_ATTEMPTS:
                attempt_data["lock_until"] = now + LOGIN_LOCK_SECONDS

            save_login_attempts(client_ip, username, attempt_data)

            if attempt_data["lock_until"] > now:
                return Response(
                    content="Too many failed attempts. Try again in 10 minutes.",
                    status_code=429,
                    headers={"Retry-After": str(LOGIN_LOCK_SECONDS)}
                )

            remaining_attempts = LOGIN_MAX_ATTEMPTS - attempt_data["failures"]

            return Response(
                content=(
                    f"You have {remaining_attempts} attempts remaining."
                    if remaining_attempts > 1
                    else "You have 1 attempt remaining."
                ),
                status_code=401
            )
...
<script>
function samaEscapeHtml(v){return String(v??'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;');}
async function doLogin(event){
  ...
  if(r.ok){
    ...
    errorBox.textContent = message || 'Invalid username or password.';
...
async def import_excel(request: Request, filename: str = "", update_existing: int = 0, preview_only: int = 0, skip_warnings: int = 0):
    ...
    if content_length_header:
        try:
            if int(content_length_header) > MAX_EXCEL_UPLOAD_BYTES:
                raise HTTPException(413, "Excel dosyasi 50 MB sinirini asiyor.")
        except ValueError:
            raise HTTPException(400, "Gecersiz Content-Length.")
    raw = await request.body()
    if len(raw) > MAX_EXCEL_UPLOAD_BYTES:
        raise HTTPException(413, "Excel dosyasi 50 MB sinirini asiyor.")
    if not raw:
...
# _SAMA_SAFE_ONEDRIVE_V3
MAX_ONEDRIVE_DOWNLOAD_BYTES = 50 * 1024 * 1024

def _safe_onedrive_url(url):
    from urllib.parse import urlparse
    p = urlparse((url or '').strip())
    host = (p.hostname or '').lower().rstrip('.')
    if p.scheme != 'https' or not host:
        raise HTTPException(400, 'OneDrive baglantisi HTTPS olmali.')
    ok = (host == '1drv.ms' or host == 'onedrive.live.com' or host == '1drv.com' or host.endswith('.1drv.com') or host == 'sharepoint.com' or host.endswith('.sharepoint.com'))
    if not ok:
        raise HTTPException(400, 'Gecersiz OneDrive/SharePoint adresi.')
    return url

def _onedrive_safe_get(session, url, timeout=45, max_redirects=6):
    from urllib.parse import urljoin
    current = _safe_onedrive_url(url)
    for _ in range(max_redirects + 1):
        ...
        if 300 <= r.status_code < 400:
            ...
            r.close()
            if not loc:
                raise HTTPException(502, 'OneDrive yonlendirmesi gecersiz.')
            current = _safe_onedrive_url(urljoin(current, loc))
            continue
        size = r.headers.get('Content-Length')
        if size:
            try:
                if int(size) > MAX_ONEDRIVE_DOWNLOAD_BYTES:
                    r.close()
                    raise HTTPException(413, 'OneDrive dosyasi 50 MB sinirini asiyor.')
            except ValueError:
                pass
        if r.status_code < 400:
            data = r.content
            if len(data) > MAX_ONEDRIVE_DOWNLOAD_BYTES:
                r.close()
                raise HTTPException(413, 'OneDrive dosyasi 50 MB sinirini asiyor.')
            r._content = data
        return r
    raise HTTPException(502, 'OneDrive yonlendirme limiti asildi.')
...
def _public_onedrive_download(share_url: str):
    ...
    for url in variants:
        try:
            r=_onedrive_safe_get(session,url,timeout=45)
            attempts.append({
                "url":url,
            ...
            if r.ok and ("text/html" in ct or r.content.lstrip().lower().startswith(b"<!doctype html")):
                ...
                for candidate in candidates:
                    ...
                    seen.add(candidate)
                    try:
                        dr=_onedrive_safe_get(session,candidate,timeout=60)
                        attempts.append({
                            "url":"HTML_CANDIDATE",
...
async function openDashboardFleet(state){
  ...
  ${rows.map(x=>`<tr>
    <td><b>${x.plate||''}</b></td><td>${samaEscapeHtml(x.driver||'')}</td><td>${x.scna||''}</td>
    <td>${samaEscapeHtml(x.area||'')}</td><td>${samaEscapeHtml(x.vessel||'')}</td><td>${integratedStateLabel(x.operation_state)}</td>
...
async function loadDash(){
  ...
  d.forEach(x=>{
    ...
    dashRows.innerHTML+=`<tr>
      <td><b>${samaEscapeHtml(x.scna)}</b></td><td>${samaEscapeHtml(x.plate)}</td><td>${money(x.actual_km)}</td>
...
function renderCurrentTripRows(){
 ...
 d.forEach(x=>{
    tripRows.innerHTML+=`<tr>
      <td data-col="scna"><b>${samaEscapeHtml(x.scna)}</b></td>
      <td data-col="plate">${samaEscapeHtml(x.plate)}</td>
      <td data-col="driver">${samaEscapeHtml(x.driver_name||x.driver||"")}</td>
      <td data-col="customer">${samaEscapeHtml(x.customer_name||x.customer||"")}</td>
      <td data-col="area">${samaEscapeHtml(x.area_name||x.area||"")}</td>
      <td data-col="vessel">${samaEscapeHtml(x.vessel_name||'')}</td>
      <td data-col="cargo">${samaEscapeHtml(x.cargo_name||x.cargo||"")}</td>
      <td data-col="cargo_type">${samaEscapeHtml(x.cargo_type_name||x.cargo_type||'')}</td>
      ...
      <td data-col="status">
        <div style="display:flex;gap:8px;align-items:center">
          <select onchange="changeTripStatus('${samaEscapeHtml(x.scna)}',this.value)">
          ...
          <button class="btn secondary" onclick="openEditFrom('${samaEscapeHtml(x.scna)}')">Düzenle</button>
        </div>
      </td></tr>`;
 });
 applyColumnPrefs();
...
async function loadExit(){
  ...
  d.forEach(x=>{
    exitRows.innerHTML+=`<tr>
      <td><b>${samaEscapeHtml(x.scna)}</b></td>
      <td>${samaEscapeHtml(x.plate)}</td>
      <td>${money(x.freight_total)}</td>
      <td>${fmtDateTime(x.exit_at)}</td>
      <td>${x.status}</td>
      <td><button class="btn orange" onclick="openExit('${samaEscapeHtml(x.scna)}')">${x.exit_done?'Çıkışı Düzenle':'Çıkış Yap'}</button></td>
    </tr>`;
  });
}
...
async function loadEntry(){
  ...
  d.forEach(x=>{
    entryRows.innerHTML+=`<tr>
      ...
      <td>${money(x.freight_total)}</td>
      <td><button class="btn green" onclick="openEntry('${samaEscapeHtml(x.scna)}')">${x.entry_done?'Girişi Düzenle':'Giriş Yap'}</button></td>
    </tr>`;
  });
}
...
function renderQuality(){
  ...
  rows.forEach(x=>{
    qualityRows.innerHTML+=`<tr class="alert-orange">
      <td><b>${samaEscapeHtml(x.scna)}</b></td><td>${x.plate||''}</td><td>${x.driver_name||''}</td>
      ...
      <td><button class="quick-btn" onclick="openEditFrom('${samaEscapeHtml(x.scna)}')">Düzelt</button></td></tr>`;
  });
}
...
function renderCompare(){
  ...
  rows.forEach(x=>{
    h+=`<tr class="${x.result==='FARKLI'?'alert-orange':'alert-red'}">
      ...
      <td>${x.result!=='DB_VAR_EXCEL_YOK'?`<button class="quick-btn" onclick="openEditFrom('${samaEscapeHtml(x.scna)}')">DB Kaydını Aç</button>`:''}</td></tr>`;
  });
  ...
  try{
    ...
    let html=`
      <div class="detail-grid">
        <div class="detail-box"><span>SCNA</span><strong>${samaEscapeHtml(x.scna)}</strong></div>
...
async function loadDailySummary(){
  ...
  if(d.rows.length){
    d.rows.forEach(x=>{
      h+=`<tr><td><b>${samaEscapeHtml(x.scna)}</b></td><td>${x.plate||''}</td><td>${x.driver_name||''}</td>
...
function renderOperationCenter(){
  ...
  rows.forEach(x=>{
    ...
    opRows.innerHTML+=`<tr class="${x.delayed?'alert-red':''}">
      <td><b>${samaEscapeHtml(x.scna)}</b></td>
      <td><button class="quick-btn" onclick="openVehicleCardFrom('${samaEscapeHtml(x.plate)}')">${x.plate||''}</button></td>
...
function renderFleetManage(){
  ...
  fleetManageRows.innerHTML=a.map(x=>`<tr><td><b>${samaEscapeHtml(x.plate)}</b></td><td>${x.brand||''}</td><td>${x.model||''}</td><td>${x.vehicle_type||''}</td><td>${x.current_driver||''}</td><td>${x.garage_state||''}</td><td>${x.is_active?'AKTİF':'PASİF'}</td><td>${x.note||''}</td><td><button class="btn secondary" onclick="openFleetVehicle('${samaEscapeHtml(x.plate)}')">Düzenle</button> <button class="btn ${x.is_active?'orange':'green'}" onclick="toggleFleetVehicle('${samaEscapeHtml(x.plate)}',${x.is_active?0:1})">${x.is_active?'Pasife Al':'Aktif Et'}</button></td></tr>`).join('');
  if(typeof v54InitTables==='function')v54InitTables();
}
...
function renderFleetOps(){
 ...
 fleetRows.innerHTML=a.map(x=>`<tr><td><b>${x.load_state==='SIRA'?(x.queue_no||'-'):'-'}</b></td><td><b>${samaEscapeHtml(x.plate)}</b></td><td>${fleetLabel(x.load_state)}</td><td>${samaEscapeHtml(x.driver||'')}</td><td>${x.scna||''}</td><td>${samaEscapeHtml(x.area||'')}</td><td>${samaEscapeHtml(x.vessel||'')}</td><td>${samaEscapeHtml(x.operation_note||'')}</td><td><button class="btn secondary" onclick="editFleetOp('${samaEscapeHtml(x.plate)}')">Durum Değiştir</button></td></tr>`).join('');
 if(typeof v54InitTables==='function')v54InitTables();
}
...
function renderVesselOps(){
  ...
  vesselRows.innerHTML=a.map(x=>{
    ...
    return `<tr>
      ...
      <td><b>${samaEscapeHtml(x.plate)}</b></td>
      <td>${samaEscapeHtml(x.driver||'')}</td>
      <td>${s==='CALISIYOR'?'GEMİDE ÇALIŞIYOR':'OPERASYONDAN ÇIKTI'}</td>
      <td>${samaEscapeHtml(x.operation_note||'')}</td>
      <td><button class="btn secondary" onclick="editVesselAssignment('${samaEscapeHtml(x.plate)}')">Düzenle</button></td>
    </tr>`;
  }).join('');
...
function renderMaintenance(){
  ...
  d.forEach(x=>{
    ...
    maintRows.innerHTML+=`<tr class="${cls}">
      <td><b>${samaEscapeHtml(x.plate)}</b></td><td>${money(x.current_km)}</td><td>${money(x.last_service_km)}</td>
      <td>${money(x.next_service_km)}</td><td>${x.next_service_km>0?money(x.km_left):''}</td>
      <td>${money(x.oil_change_km)}</td><td>${samaEscapeHtml(x.tire_note||'')}</td><td>${samaEscapeHtml(x.brake_note||'')}</td>
      <td>${samaEscapeHtml(x.engine_note||'')}</td>
...
async function loadAlerts(){
  ...
  if(d.delayed.length){
    d.delayed.forEach(x=>{
      html+=`<tr class="alert-red">
        <td><b>${samaEscapeHtml(x.scna)}</b></td><td>${samaEscapeHtml(x.plate)}</td><td>${x.driver_name||''}</td>
  ...
  try{
    ...
    if(rf.length){
      rf.forEach(x=>{
        html+=`<tr class="alert-orange"><td><b>${samaEscapeHtml(x.scna)}</b></td><td>${samaEscapeHtml(x.plate)}</td>
...
async function loadAudit(){
  ...
  d.forEach(x=>{
    ...
    <tr>
      <td>${x.created_at}</td>
      <td>${x.action}</td>
      <td>${samaEscapeHtml(x.scna)}</td>
      <td>${samaEscapeHtml(x.detail||'')}</td>
    </tr>`;
  });
  filterAuditRows();
}
    ...
    <tbody>
      ${f.map(x=>`
        <tr>
          ...
          <td>${x.note||''}</td>
          <td>
            <button class="btn danger" onclick="delFuel(${x.id},'${samaEscapeHtml(x.scna)}')">Sil</button>
          </td>
        </tr>
      `).join('')}
    </tbody>
...
class SamaCSRFMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request, call_next):
        ...
        if request.method in ("POST","PUT","PATCH","DELETE"):
            ...
            if source:
                try:
                    u=urlparse(source)
                    allowed=(u.scheme=="https" and u.hostname and u.hostname.lower()==host)
                except Exception:
                    allowed=False
            if not allowed:
                return JSONResponse(
                    {"detail":"CSRF korumasi: gecersiz istek kaynagi."},
                    status_code=403,
                    headers={"Cache-Control":"no-store"},
                )
        return await call_next(request)

app.add_middleware(SamaCSRFMiddleware)
# SAMA_CSRF_GUARD_V1
...
def sama_safe_https_url(url):
    u=urlparse(str(url).strip())
    if u.scheme != "https" or not u.hostname:
        raise ValueError("Yalnizca HTTPS URL kabul edilir.")
    host=u.hostname.lower()
    if host == "localhost" or host.endswith(".local"):
        raise ValueError("Yerel host reddedildi.")
    try:
        for item in socket.getaddrinfo(host,None):
            ip=ipaddress.ip_address(item[4][0])
            if ip.is_private or ip.is_loopback or ip.is_link_local or ip.is_multicast or ip.is_reserved:
                raise ValueError("Ozel/yerel IP reddedildi.")
    except socket.gaierror:
        raise ValueError("Host cozumlenemedi.")
    return url
# SAMA_SSRF_GUARD_V1
